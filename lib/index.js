	...
	self.getComponent = function(args) {
		...
		if (self.lifecycle == "singleton") {
			...
			if (_.isNull(self.singleton)) {
				...
				instance = new Object();
				self.singleton = self.type.apply(instance, dependencies);
...
function Container($scarlet) {
	...
	var resolveForKey = function(keyName, args) {
		...
		assert(instance, "No instance created for key '{0}'".format(keyName));

		if (!_.isNull(instance) && !_.isNull(self.interceptor)){
			self.scarlet.intercept(instance).using(self.interceptor);
		}

		return instance;
	};

	var resolveForType = function(type, args) {
		...
		assert(instance, "No instance created for type '{0}'".format(typeof(type)));
	...
	self.resolve = function(keyOrType, args) {

		if (typeof(keyOrType) == "string") {
			return resolveForKey(keyOrType, args);
		}

		return resolveForType(keyOrType, args);
	}
}
