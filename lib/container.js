...
function Container($scarlet, $ioc) {
	...
	self.resolveForType = function(type, args) {
		...
		ioc.assert(component, "Could not resolve instance using type '{0}'".format(typeof(type)));

		var instance = component.getComponent(args);

		ioc.assert(instance, "No instance created for type '{0}'".format(typeof(type)));
	...
	self.resolve = function(keyOrType, args) {
		...
		ioc.assert(keyOrType, "Cannot resolve with keyOrType = 'null'");

		if (typeof(keyOrType) == "string") {
			return self.resolveForKey(keyOrType, args);
		}

		return self.resolveForType(keyOrType, args);
	}
}
